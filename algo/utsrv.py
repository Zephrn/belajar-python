...
resep = f"Receipt No.\t{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}"
print(resep.center(panjang))
print(DINEIN_OR_T.center(panjang))
print("")
print("="*panjang)
...
for item, pesanan_detail in pesanan.items():
    if item in makanan:
        ...
        print('%-1s %-22s %20s' % (pesanan_detail['jumlah'], makanan[item]['name'], harga_item * pesanan_detail['jumlah']))
    elif item in minuman:
        harga_item = minuman[item]['price']
        print('%-1s %-22s %20s' % (pesanan_detail['jumlah'], minuman[item]['name'], harga_item * pesanan_detail['jumlah']))
    else:
